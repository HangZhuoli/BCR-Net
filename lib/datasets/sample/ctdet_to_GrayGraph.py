...
class CTDetDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        #增加灰度图进行处理，添加处理灰度图的输入
        sharp_img = cv2.imread(sharp_img_path,cv2.IMREAD_GRAYSCALE)
        blur_img = cv2.imread(blur_img_path,cv2.IMREAD_GRAYSCALE)
        sharp_img = cv2.cvtColor(sharp_img,cv2.COLOR_GRAY2RGB)
