...
class opts(object):
    def __init__(self):
        ...
        """
        1、修改dataset参数，选择添加针对陨石坑处理的单一目标检测参数crater
        """
        self.parser.add_argument('--dataset', default='visdrone', help='visdrone | uavdt | crater') #如果选择新的数据集需要对这个参数进行修改
        ...
        # model
        # 添加了针对陨石坑处理的新的模型网络DREB_Net_crater网络结构
        self.parser.add_argument('--arch', default='DREB_Net', help='model architecture. Currently tested Exist DREB_Net_crater')
    ...
    def parse(self, args=''):
        if args == '':
            opt = self.parser.parse_args() # 当程序没有传递任何参数的时候，选择所有的默认的参数化构造
        else:
            opt = self.parser.parse_args(args) #选择带有参数的初始化创建
    ...
    def update_dataset_info_and_set_heads(self, opt, dataset):
        """
        1、选择输入的input_h以及input_w是由用户自定义的，我选择使用256，与原始图像保持一致，因为涉及到原始图片标注狂的缩放
        """
