...
class DREB_Net(nn.Module):
    ...
    def __init__(self, num_blocks=[4, 6, 16, 1], width_multiplier=[1, 1, 1, 1], override_groups_map=None, deploy=False, use_checkpoint=False,
        ...
        # 将原始的维度从128修改为32，因为我输入的尺寸为256*256.而不是之前的1024*1024
        self.LFAMM = LFAMM(channels=128, height=128, weight=128)

        """
        1、论文实现的是两个分支，这一分支代表着目标检测分支，用于实现对于特征图的不断下采样的操作实现特征图的提取
        其中stage操作为对特征图实现下采样操作，deconv为论文中提到的DetectConv，为上采样操作
        """
        ...
        """
        2、论文实现的是两个分支，这一分支代表着去模糊分支，实现了类似Unet网络结构来实现从运动模糊图像中提取特征的操作
        """
        ...
        """
        head定义：
        1、自定义目标识别的检测头，主要通过自定义检测HM损失，WH损失，以及REG损失来实现对于图像最终结果的预测
        """

        for head in sorted(self.heads):
            # 最终输出的形式【B,C,H,W】,C代表检测的类别，如果设置为陨石坑数据集的话，其值选择为1
    ...
    def forward(self, x, mode):
        ...
        if mode == 'val':
            # 会直接跳过后续的训练部分从而直接进入检测阶段，属于论文中所提到的方法，摒弃更多的模糊特征操作，实现更快的推理
            # 因为核心目标还是在运动模糊的图像中实现目标检测
            return [ret]
        
        # 在训练阶段需要对模糊图像尝试进行恢复，所以网络在训练阶段会变深，实现更好的模糊图像检测效果
        elif mode == 'train':
            deblur_inp = s2
