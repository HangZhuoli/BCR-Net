...
def save_feature_map(tensor, name, save_dir="debug_feats"):
    """
    tensor: BCHW
    自动处理 batch>1，通道>1 的情况
    """
    os.makedirs(save_dir, exist_ok=True)

    # 取 batch 中的第一张图
    fmap = tensor[0].detach().cpu()   # CHW

    # 对通道求平均 → H×W
    fmap = fmap.mean(0).numpy()

    # 保障图像是 2D（H×W）
    assert fmap.ndim == 2, f"Feature map must be 2D, got shape {fmap.shape}"
...
class DREB_FPN_Net_SSAttn_v2(nn.Module):
    ...
    def forward(self, x, mode='train'):
        ...
        save_feature_map(deblur_img, "deblur_img")
        ...
        for head_name, head_out in ret.items():
            save_feature_map(head_out, f"detect_{head_name}")
